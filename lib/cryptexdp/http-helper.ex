defmodule Cryptexdp.HttpHelper do
  @moduledoc """
  Helper
  """
  import Cryptexdp.RetryHelper
  alias Cryptexdp.RetryHelper, as: PoisonRetryHelper
  alias Cryptexdp.UriHelper
  ...
  def call_apis_async(url_list) do
    ...
    cyan_text("time #{seconds}s, #{inspect(self())} domain: #{UriHelper.get_domain(hd(url_list))}, url count: #{length(url_list)}") |> IO.puts()
